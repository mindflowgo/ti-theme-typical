            ...
            <div class="overflow-auto">
                ...
                @if ($tipAmounts = $__SELF__->tippingAmounts())
                    <div class="btn-group btn-group-toggle w-100 text-center tip-amounts">
                        <button
                            type="button"
                            class="btn btn-outline-primary text-nowrap{{ $tipAmountType == 'none' ? ' active' : '' }}"
                        ...
                        @foreach ($tipAmounts as $tipAmount)
                            <button
                                type="button"
                                class="btn btn-outline-primary text-nowrap{{ $currentAmount == $tipAmount->value ? ' active' : '' }}"
                                data-cart-control="tip-amount"
                        ...
                        <button
                            type="button"
                            class="btn btn-outline-primary{{ $tipAmountType == 'custom' ? ' active' : '' }}"
                            data-cart-control="tip-amount"
